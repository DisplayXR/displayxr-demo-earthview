 ...
 *
 * ---------------------------------------------------------------------------
 * VENDORED into displayxr-demo-earthview from
 *   DisplayXR/displayxr-runtime  test_apps/common/dxr_view_config.h
 *   @ c1e4fe00da0f189122eca14e61e9faaa88e4b38e
 * It lives HERE (openxr_includes/, not openxr_includes/openxr/) on purpose:
 * VENDORED.json pins every *.h in openxr_includes/openxr/ to
 * src/external/openxr_includes/openxr/<name>, and this file has a different
 * home in the runtime, so it would fail that check as an unpinnable header.
 * openxr_includes/ is already on all four legs' include paths (windows/,
 * macos/, linux/ CMakeLists + android CMakeLists), so `#include
 * <dxr_view_config.h>` resolves everywhere.
 *
 * One deliberate deviation from the runtime copy — the second-stage
 * `#ifndef` below. Everything else is verbatim; re-copy, don't hand-edit.
 * ---------------------------------------------------------------------------
 */
...
/*
 * VENDORED-COPY DEVIATION (displayxr-demo-earthview).
 *
 * Second-stage fallback, not present in the runtime's copy. The probe above
 * finds openxr_includes/openxr/XR_DXR_display_info.h on every leg's include
 * path, so the `#else` arm never fires here — but that header is VENDORED and
 * pinned per file (openxr_includes/VENDORED.json). A future refresh that
 * rolls it BACK to a pre-v19 revision, or a partial re-vendor, would satisfy
 * __has_include and still not define the enumerator, and the build would break
 * with no obvious cause. So define it here if, after the include, it is still
 * missing. `defined()` is exact: v19 spells the value as a macro, so a header
 * that already carries it suppresses this, and if the enumerator is ever
 * registered with Khronos as a real enum, the header that carries it will also
 * carry the macro (the runtime's own copy does) — this arm still will not fire.
 */
