...
// rig switch (disturbance-free converter), frame the picked POI onto the
// zero-parallax plane via targetDist, and orbit the CAMERA around it. World stays
// camera-centric (no diorama); g_geoNav.orbitAcquired is NOT used in this path.
                ...
                if (frameState.shouldRender) {
                    if (LocateViews(*xr, frameState.predictedDisplayTime,
                        ...
                        if (useRig) {
                            ...
                            // the 1.36x zoom the cube C-toggle exposed).
                            ...
                            if (rigCamera) {
                                ...
                                if (g_stereoFull > 1.0e-4) {
                                    ...
                                    cameraRig.ipdFactor =
                                        (float)((1.0 - sf) * inputSnapshot.viewParams.ipdFactor + sf * invF);
                                    cameraRig.parallaxFactor =
                                        (float)((1.0 - sf) * inputSnapshot.viewParams.parallaxFactor + sf * invF);
                                }
                                ...
                                if (g_focusActive) {
                                    ...
                                    // (disturbance-free, so the cam->display switch is seamless).
                                ...
                                // [FOCUS] NO rig-pose translation when NOT focused (orbit/diorama
                                // keeps identity). The POI is placed on the convergence plane purely
                                // by the targetDist framing, so the display eye stays at the origin
                                // and the depth pick stays accurate.
                                // Frustum-source viewing distance = eye->display plane = persp *
                                // m2v * N (m2v = vH/canvasH); matches the cam rig's 1/convergence.
                                {
                        ...
                        // [FOCUS] runtime-resolved canvas size (m) drives the cam->display
                        // rig converter's physical_height_m next frame (canvas, not display).
                            ...
                            if (tilesActive) {
                                ...
                                if (g_pendingPick) {
                                    ...
                                    if (pickHits > 0) {
                                        ...
                                        // [FOCUS] (macOS parity) Smoothly re-aim the camera to
                                        // center the new POI and reframe it onto the zero-parallax
                                        // plane (targetDist = eye->POI / vDist => POI at XR depth
                                        // vDist). Camera POSITION stays put; only dir + zoom lerp
                                        // (g_focusT). Eye stays at the origin (no rig translation)
                                        // => the pick stays as accurate as fly mode.
                                ...
                                RenderPlaceholder(vkDevice, graphicsQueue, renderCmdPool,
                                    (*swapchainVkImages)[imageIndex], (VkFormat)xr->swapchain.format,
                                    xr->swapchain.width, xr->swapchain.height);
                            }
