...
#                                            _package/DisplayXREarthView-*.pkg
#
# Env:
...
# Bundled attribution logo + manifest + icons live in the repo. (No API key
# is ever staged — key supply is env var / earthview.ini, PRD §8.)
if [ -f "$REPO_ROOT/macos/assets/google_logo.png" ]; then
    cp "$REPO_ROOT/macos/assets/google_logo.png" "$ARTIFACT_DIR/assets/"
fi
...
mkdir -p "$PKG_DIR"
PKG_PATH="$PKG_DIR/DisplayXREarthView-${VERSION}.pkg"
